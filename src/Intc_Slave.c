...
#define SPIO_INTERRUPT_ID XPS_SPI0_INT_ID
...
XSpiPs Spi;
static void SetupInterruptSystem(XScuGic *GicInstancePtr, XGpioPs *Gpio, u16 SpioIntrId, XSpiPs *Spi);
...
XSpiPs_Enable((&Spi));
SetupInterruptSystem(&Intc, &Gpio, SPIO_INTERRUPT_ID, &Spi);
...
*/
XScuGic_Connect(GicInstancePtr, SpioIntrId,
(Xil_ExceptionHandler)XSpiPs_InterruptHandler,
(void *)Spi);


//Enable the interrupt for the SpiO device.
XScuGic_Enable(GicInstancePtr, SpioIntrId);
